...
class ShapeConstraintLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : ConstraintLayout(context, attrs, defStyleAttr) {
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.ShapeConstraintLayout).apply {
            shapeMode = getInt(R.styleable.ShapeConstraintLayout_shapeMode, 0)
            fillColor = getColor(R.styleable.ShapeConstraintLayout_shapeFillColor, 0xFFFFFFFF.toInt())
            strokeColor = getColor(R.styleable.ShapeConstraintLayout_shapeStrokeColor, 0)
            strokeWidth = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_shapeStrokeWidth, 0)
            shapeCornerRadius = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_shapeCornerRadius, 0)
            cornerPosition = getInt(R.styleable.ShapeConstraintLayout_shapeCornerPosition, -1)
            startColor = getColor(R.styleable.ShapeConstraintLayout_shapeStartColor, 0xFFFFFFFF.toInt())
            endColor = getColor(R.styleable.ShapeConstraintLayout_shapeEndColor, 0xFFFFFFFF.toInt())
            gradientOrientation = getColor(R.styleable.ShapeConstraintLayout_shapeOrientation, 0)
            elevationSize = getDimensionPixelSize(R.styleable.ShapeConstraintLayout_shapeElevation, -1)
            recycle()
        }
    }
