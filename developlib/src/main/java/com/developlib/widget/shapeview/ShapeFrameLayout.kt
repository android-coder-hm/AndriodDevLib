...
class ShapeFrameLayout @JvmOverloads constructor(context: Context, attrs: AttributeSet? = null, defStyleAttr: Int = 0) : FrameLayout(context, attrs, defStyleAttr) {
    ...
    init {
        context.obtainStyledAttributes(attrs, R.styleable.ShapeFrameLayout).apply {
            shapeMode = getInt(R.styleable.ShapeFrameLayout_shapeMode, 0)
            fillColor = getColor(R.styleable.ShapeFrameLayout_shapeFillColor, 0xFFFFFFFF.toInt())
            strokeColor = getColor(R.styleable.ShapeFrameLayout_shapeStrokeColor, 0)
            strokeWidth = getDimensionPixelSize(R.styleable.ShapeFrameLayout_shapeStrokeWidth, 0)
            shapeCornerRadius = getDimensionPixelSize(R.styleable.ShapeFrameLayout_shapeCornerRadius, 0)
            cornerPosition = getInt(R.styleable.ShapeFrameLayout_shapeCornerPosition, -1)
            startColor = getColor(R.styleable.ShapeFrameLayout_shapeStartColor, 0xFFFFFFFF.toInt())
            endColor = getColor(R.styleable.ShapeFrameLayout_shapeEndColor, 0xFFFFFFFF.toInt())
            gradientOrientation = getColor(R.styleable.ShapeFrameLayout_shapeOrientation, 0)
            elevationSize = getDimensionPixelSize(R.styleable.ShapeFrameLayout_shapeElevation, -1)
            recycle()
        }
    }
